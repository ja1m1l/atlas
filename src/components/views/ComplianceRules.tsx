...
import { FileCode2, Play, Hash, ShieldBan, ShieldCheck, Check, AlertOctagon } from 'lucide-react';

const MOCK_FORBIDDEN = ['guaranteed return', 'risk-free', '100% safe', 'foolproof', 'cure', 'magic', 'instant profit'];
const MOCK_DISCLAIMERS = [
  'Past performance is not indicative of future results.',
  'Consult your physician before beginning any wellness program.',
  'Investments involve risk including possible loss of principal.'
];

const PRESET_JSON = `{
  "policy": "Strict_FSI_v1.4.2",
  "enforce_disclaimers": true,
  "flag_threshold": 0.85,
  "blocked_terms": [
    "guaranteed return*",
    "risk-free"
  ],
  "required_entities": ["SEC", "FINRA"]
}`;
...
export function ComplianceRules() {
  ...
  const handleTest = () => {
    ...
    setTimeout(() => {
      ...
      const violations = MOCK_FORBIDDEN.filter(term => lower.includes(term));
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 flex-1 min-h-[500px]">
        ...
        <div className="flex flex-col gap-4">
          <div className="bg-white dark:bg-[#141417] border border-slate-200 dark:border-[#27272a]/60 rounded-2xl flex flex-col h-[280px] shadow-sm dark:shadow-none ring-1 ring-inset ring-transparent dark:ring-white/5 overflow-hidden group hover:border-indigo-200 dark:hover:border-zinc-700/60 transition-colors">
            ...
            <div className="flex-1 bg-white dark:bg-[#141417] p-4 font-mono text-[13px] text-slate-600 dark:text-zinc-400 overflow-y-auto leading-relaxed outline-none" contentEditable spellCheck="false" suppressContentEditableWarning={true}>
              <pre><code>{PRESET_JSON}</code></pre>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-[#141417] border border-slate-200 dark:border-[#27272a]/60 rounded-2xl p-6 flex-1 flex flex-col shadow-sm dark:shadow-none ring-1 ring-inset ring-transparent dark:ring-white/5">
            <h2 className="text-[13px] font-medium text-slate-700 dark:text-zinc-300 mb-6 flex items-center gap-2">
            ...
            <div>
              <h3 className="text-[10px] font-mono text-slate-500 dark:text-zinc-500 mb-3 uppercase tracking-widest font-semibold flex items-center gap-1.5">
                 <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 dark:bg-emerald-400" /> Required Declarations
              </h3>
              <div className="flex flex-col gap-2">
                {MOCK_DISCLAIMERS.map((desc, i) => (
                  <div key={i} className="px-3 py-2 bg-slate-50 dark:bg-[#18181b] border border-slate-200 dark:border-[#27272a]/60 text-slate-600 dark:text-zinc-400 text-[12px] rounded-lg font-sans shadow-sm leading-relaxed">
                    {desc}
                  </div>
                ))}
              </div>
