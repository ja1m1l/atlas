...
export function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 font-sans overflow-hidden flex flex-col items-center p-2 sm:p-6 relative ${isDarkMode ? 'text-[#e4e4e7]' : 'text-slate-900'}`}>
      ...
      {/* Background Layers */}
      <div className={`fixed inset-0 transition-colors duration-300 ${isDarkMode ? 'bg-[#050505]' : 'bg-[#f0f2f5]'}`} />
      ...
      <header className="flex w-full items-center justify-between px-4 pb-6 max-w-[1440px] pt-4 sm:pt-0 relative z-20">
        ...
        <div className="flex items-center gap-6">
          <button 
            ...
            className="flex items-center gap-2 text-[11px] font-mono uppercase font-bold tracking-widest text-emerald-600 dark:text-emerald-400 hover:opacity-80 transition-colors mr-2 px-4 py-2 bg-white dark:bg-emerald-500/10 rounded-full border border-slate-200 dark:border-emerald-500/20 shadow-sm dark:shadow-none"
          >
              ...
              <button 
                ...
                className={`w-8 h-8 rounded-full border flex items-center justify-center transition-colors shadow-sm overflow-hidden ${
                  isProfileOpen 
                    ...
                    : 'bg-slate-50 dark:bg-zinc-800 border-slate-200 dark:border-white/10 text-slate-500 dark:text-zinc-400 hover:bg-slate-200 dark:hover:bg-zinc-700'
                }`}
              >
      ...
      <main className="w-full flex-1 max-w-[1440px] bg-[#fdfdfd] dark:bg-[#0c0c0e]/80 backdrop-blur-3xl rounded-3xl border border-slate-200 dark:border-[#27272a]/40 shadow-xl dark:shadow-2xl overflow-hidden ring-1 ring-inset ring-slate-100 dark:ring-white/5 flex flex-col relative z-10 h-[calc(100vh-8rem)] transition-colors duration-300">
